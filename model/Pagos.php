...
class Pagos
{
  ...
  {
    try {
      $query = "SELECT * FROM pagos_estudiante WHERE estudiante_periodo_id = :id ORDER BY fecha_pago ASC, id ASC";
  ...
  {
    try {
      // Subconsulta para obtener el último pago por inscripción
      $subquery = "SELECT pe1.*
                   ...
                   ON ult.estudiante_periodo_id = pe1.estudiante_periodo_id AND ult.max_id = pe1.id";
      ...
      $query = "SELECT ep.id AS estudiante_periodo_id,
                ...
                LEFT JOIN ($subquery) ue ON ue.estudiante_periodo_id = ep.id
  ...
  {
    try {
      $query = "SELECT pe.*, e.cedula, e.nombre, e.apellido, c.id AS id_clase, i.name AS nombre_idioma, p.id AS periodo_id FROM pagos_estudiante pe INNER JOIN estudiante_periodo ep ON ep.id = pe.estudiante_periodo_id INNER JOIN estudiantes e ON e.cedula = ep.estudiante_id INNER JOIN Periodo p ON p.id = ep.periodo_id INNER JOIN clases c ON c.id = p.idClase INNER JOIN idiomas i ON i.id = c.idioma ORDER BY e.apellido, e.nombre, pe.fecha_pago DESC, pe.id DESC";
      $max = (int)$limit?? 10;
      $min = (int)$offset ?? 0;
