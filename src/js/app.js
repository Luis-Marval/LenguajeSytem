...
class ThemeCustomizer {
  ...
  changeThemeMode(color) {
    ...
    localStorage.setItem("data-mode", color);
    this.setSwitchFromConfig();
  }
  ...
  changeSidenavView(view) {
    this.html.setAttribute("data-sidenav-view", view);
    localStorage.setItem("data-sidenav-view", view);
...
class notificationsMessage {
  ...
  format() {
    if (this.type == "success") {
      return `<div id="notificacion-alert" class="transition duration-300 bg-success/25 border border-teal-200 rounded-md p-4 mb-2" role="alert"><div class="flex items-center gap-3"><div class="flex-shrink-0"> <i class="mgc_-badge-check text-xl"></i>
      </div><div class="flex-grow"><div class="text-sm text-success font-medium"><span class="font-bold">Alerta:</span>${this.message}</div></div><button data-fc-dismiss="dismiss-alert" type="button" id="dismiss-notification" class="ms-auto h-8 w-8 rounded-full bg-green-400 flex justify-center items-center"><i class="mgc_close_line text-xl text-green-800"></i></button></div></div>`;
    }
    if (this.type == "error") {
      return `<div id="dismiss-alert" class="transition duration-300 bg-danger/25 border border-danger rounded-md p-2 m-1 col-span-3" role="alert"><div class="flex items-center gap-3"><div class="flex-shrink-0"><i class="mgc_-badge-check text-xl"></i></div><div class="flex-grow"><div class="text-sm text-danger font-medium"><span class="font-bold">Error: </span> ${this.message}</div></div><button data-fc-dismiss="dismiss-alert" type="button" id="dismiss-notification" class="ms-auto h-8 w-8 rounded-full bg-red-400 flex justify-center items-center hover:bg-red-500"><i class="mgc_close_line text-xl text-red-800"></i></button></div></div>`;
    }
  }

  close(container) {
    const btn = container.querySelector(
      '[data-fc-dismiss="dismiss-alert"], #dismiss-notification',
    );
    if (btn) {
      btn.addEventListener("click", function (e) {
        e.preventDefault();
        const alertEl = container.querySelector(
          "#notificacion-alert, #dismiss-alert",
        );
        if (alertEl) {
...
if (!localStorage.hasOwnProperty("data-mode")) {
  localStorage.setItem("data-mode", "light");
}

document.addEventListener("DOMContentLoaded", function () {
  const rowForPage = document.getElementById("rowForPage");
  if (rowForPage) {
    (function () {
      const params = new URLSearchParams(window.location.search);

      if (params.has("PorPagina")) {
        const val = params.get("PorPagina");
        if ([...rowForPage.options].some((o) => o.value === val)) {
          rowForPage.value = val;
        }
      } else {
        rowForPage.value = 10;
      }
    ...
    rowForPage.addEventListener("change", function () {
      // actualizar params y navegar (envía GET al back)
      const newParams = new URLSearchParams(window.location.search);
      newParams.set("PorPagina", rowForPage.value);
  ...
  if (before) {
    before.addEventListener("click", function () {
      // actualizar params y navegar (envía GET al back)
      const newParams = new URLSearchParams(window.location.search);
      if (newParams.has("pagina")) {
        if (parseInt(newParams.get("pagina")) > 1) {
          newParams.set("pagina", parseInt(newParams.get("pagina")) - 1);
          window.location.search = newParams.toString();
        }
      }
    });
  ...
  if (after) {
    after.addEventListener("click", function () {
      const newParams = new URLSearchParams(window.location.search);
      if (newParams.has("pagina")) {
        if (parseInt(newParams.get("pagina")) < pages) {
          newParams.set("pagina", parseInt(newParams.get("pagina")) + 1);
          window.location.search = newParams.toString();
        }
      } else if (page < pages) {
        newParams.set("pagina", 2);
        window.location.search = newParams.toString();
      }
    });
  }
